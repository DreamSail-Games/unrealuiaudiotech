...
static void engine_handle_cmd(struct android_app* app, int32_t cmd)
{
   switch (cmd)
   {
		...
		case APP_CMD_INIT_WINDOW:
		{
			...
			platformInitSettings.jNativeActivity = app->activity->clazz;			
			IntegrationDemo::Instance().GetLowLevelIOHandler()->InitAndroidIO(app->activity->vm, app->activity->clazz);

			if( !IntegrationDemo::Instance().Init( memSettings, stmSettings, deviceSettings, initSettings, platformInitSettings, musicInit, NULL, errBuffer, 256,width, height) )
			{
				LOGI("%s", errBuffer);
			}
			else
			{
				...
				IntegrationDemo::Instance().GetLowLevelIOHandler()->AddBasePath("/sdcard");	// Add writable path for AkRecorder plug-in.
				IntegrationDemo::Instance().GetLowLevelIOHandler()->AddBasePath( app->activity->internalDataPath );				
			}
		}
			break;
...
void android_main(struct android_app* state)
{
    ...
    while (1)
    {
        ...
        {			
			ANativeWindow_Buffer buffer;
			ANativeWindow_lock( state->window, &buffer, NULL);
